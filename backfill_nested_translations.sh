...
translate_text() {
    ...
    if [ -n "$GOOGLE_TRANSLATE_API_KEY" ] && [ "$GOOGLE_TRANSLATE_API_KEY" != "your_api_key_here" ]; then
        ...
        local response=$(curl -s -X POST \
            ...
            "$url")
        translation=$(echo "$response" | python3 -c "import sys, json; print(json.load(sys.stdin).get('data', {}).get('translations', [{}])[0].get('translatedText', ''))" 2>/dev/null)
    fi
    
    # Fallback to free API
    if [ -z "$translation" ]; then
        local fallback_url="https://translate.googleapis.com/translate_a/single?client=gtx&sl=en&tl=ar&dt=t&q=${encoded}"
        local fallback_response=$(curl -s "$fallback_url")
        translation=$(echo "$fallback_response" | python3 -c "import sys, json; print(''.join([t[0] for t in json.load(sys.stdin)[0]]))" 2>/dev/null)
    fi
