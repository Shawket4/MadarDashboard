...
> For a fresh agent continuing the Madar dashboard UI rebuild. Read this top to bottom before touching anything. Branch: `audit/overnight`. Nothing is committed yet (see **Commit status**).

## 1. The goal

Boldly reimagine **all** of the Madar dashboard UI, surface by surface, to the standard set in `PRODUCT.md` + `DESIGN.md` ("The Warm Ledger"). The cadence is: reimagine one surface → verify it across the full matrix → show the user → move on.
...
| Backend delivery endpoint | `GET /reports/branches/{branch_id}/delivery-sales` added to MadarRust (see §6). |
...
- Run it via the Preview MCP: `.claude/launch.json` defines server **`madar-mock`** (`npm run dev:mock`, port 5180). `preview_start("madar-mock")`.
...
- **Theme/lang/scroll:** theme is class-driven, persisted at `localStorage["madar.theme"]` (`light`/`dark`/`system`); OS colorScheme emulation doesn't reliably flip it — set the key + reload. Language is `localStorage["madar-pos.cloud"].state.language` (`en`/`ar`); set it + reload to get RTL.
...
The user **authorized backend changes** for data needs (it's the sibling repo `/Users/shawket/Desktop/MadarRust`, Actix-Web + utoipa + sqlx). Pattern (mirrors the delivery endpoint just added):
...
3. Regenerate the spec: `cd ../MadarRust && cargo run --bin export-openapi` → writes `openapi.json`.
4. Regenerate the client: `cd ../MadarDashboard && npm run generate:api` (Orval) → new hook + types + (unused) MSW + faker.
5. Curate mock data for it (§4) and consume the generated hook.
Reference impl: `branch_delivery_sales` in `MadarRust/src/reports/handlers.rs` (+ `routes.rs`, `openapi.rs`).
...
Everything else (the UI changes, i18n, generated client, backend `MadarRust/src/reports/*` + `openapi.rs` + `openapi.json`) is intended to be committed when the user gives the go. **Do not push.**

## 10. Memory

Persistent context lives in the agent memory (`MEMORY.md` index): see `ui-rebuild-constraints` (design/UX-only rule, harness + preview gotchas), `madar-overhaul-learnings`, `url-state-architecture`, `public-ordering-redesign`.
