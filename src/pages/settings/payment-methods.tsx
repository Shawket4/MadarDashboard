...
import { Plus, Search, Pencil, Banknote, CreditCard, Wallet, PieChart, Truck, QrCode, Landmark, Gift, Smartphone, Receipt, Store, Star, Link as LinkIcon, Check } from "lucide-react";
...
const PREDEFINED_COLORS = [
  "#0F172A", "#334155", "#10B981", "#059669", "#3B82F6", "#2563EB",
  "#8B5CF6", "#7C3AED", "#F59E0B", "#D97706", "#EF4444", "#DC2626",
  "#EC4899", "#DB2777"
];
...
export default function PaymentMethodsPage() {
        ...
        return (
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              <span className="font-semibold text-sm leading-tight">{getLabel(pm.name)}</span>
              {isSystemMethod(pm) && <span className="text-[10px] text-muted-foreground uppercase tracking-wider font-medium">System</span>}
            </div>
          </div>
        );
      },
  ...
  return (
    <PageShell
      ...
      description={t("settings.paymentMethodsHint", { defaultValue: "Manage payment methods available for checkout." })}
      action={
...
function PaymentMethodForm({
    ...
    if (open) {
      if (method) {
        ...
        form.reset({
          ...
          labelEn: trans?.en || "",
          labelAr: trans?.ar || "",
          color: method.color?.startsWith("#") ? method.color : PREDEFINED_COLORS[2],
  ...
  const onSubmit = (values: z.infer<typeof formSchema>) => {
    const payload = {
      org_id: orgId,
      name: values.name,
      label_translations: { en: values.labelEn, ar: values.labelAr || values.labelEn },
      color: values.color,
      icon: values.icon,
    ...
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="sm:max-w-[480px]">
        <DialogHeader>
          ...
          <DialogDescription>
            {method ? "Modify your payment method settings below." : "Create a new payment method for checkout."}
          </DialogDescription>
        </DialogHeader>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  {isSystem ? (
                    <FormDescription>System method IDs cannot be changed.</FormDescription>
                  ) : (
                    <FormDescription>A unique internal identifier (lowercase, underscores only).</FormDescription>
                  )}
                  <FormMessage />
                </FormItem>
              )}
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input {...field} placeholder="e.g. فيزا" className="text-right" dir="rtl" />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
                  ...
                  <FormControl>
                    <div className="flex flex-wrap gap-2 pt-1">
                      {PREDEFINED_COLORS.map((c) => (
                        ...
                        >
                          {field.value === c && <Check size={16} className="text-white drop-shadow-sm" />}
                        </button>
                      ))}
                    </div>
                  </FormControl>
