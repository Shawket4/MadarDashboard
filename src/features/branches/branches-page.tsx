...
export function BranchesPage() {
  ...
  const columns = useMemo<ColumnDef<Branch>[]>(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => row.original.phone ? <span className="flex items-center gap-1 font-mono text-sm"><Phone className="size-2.5" />{row.original.phone}</span> : <span className="text-sm text-muted-foreground">—</span>,
      },
      {
        ...
        cell: ({ row }) => row.original.printer_brand ? (
          <div className="flex items-center gap-1.5">
            ...
            <div><p className="text-xs font-semibold capitalize">{row.original.printer_brand}</p><p className="font-mono text-xs text-muted-foreground">{row.original.printer_ip}:{row.original.printer_port}</p></div>
          </div>
  ...
  if (!orgId) return <Page><div className="space-y-1.5"><h1 className="text-xl font-semibold tracking-tight text-balance sm:text-2xl">{t("branches.title", "Branches")}</h1></div><EmptyState icon={GitBranch} title={t("branches.pickOrg", "Select an organization")} /></Page>;

  return (
    <Page>
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div className="space-y-1.5">
          <h1 className="text-xl font-semibold tracking-tight text-balance sm:text-2xl">{t("branches.title", "Branches")}</h1>
