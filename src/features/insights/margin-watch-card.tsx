  ...
  return (
    <ChartCard title={t("insights.watch.title", "Margin watch")} contentClassName="px-4 sm:px-6">
      {watch.isLoading ? (
        ...
        <ErrorState
          title={t("insights.watch.loadFailed", "Couldn't load margin watch")}
          onRetry={() => void watch.refetch()}
          retrying={watch.isFetching}
          className="border-0 bg-transparent"
        />
      ) : empty ? (
        <EmptyState title={t("insights.watch.empty", "Margins appear here once items sell in this period")} className="border-0 bg-transparent" />
      ) : (
        <div className="space-y-4">
          {/* Headline: gross margin EGP + % with delta vs the previous period. */}
          <div className="flex flex-wrap items-baseline gap-x-2.5 gap-y-1">
            <span className="font-mono text-2xl font-semibold leading-none tracking-tight tabular">
            ...
            {marginDelta != null ? (
              <span
                className={cn(
                  ...
                  marginDelta >= 0 ? TINT.success : TINT.danger,
                )}
              >
                {marginDelta >= 0 ? (
            ...
            <li key={`${r.menu_item_id}-${r.size_label}`} className="flex items-start gap-2 text-sm">
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-baseline justify-between gap-2">
                  ...
                  <span className="shrink-0 font-mono tabular">{fmtMoney(r.margin)}</span>
                </div>
                {r.flags[0] ? (
