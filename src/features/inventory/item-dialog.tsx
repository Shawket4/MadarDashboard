...
export function ItemDialog({ orgId, open, onOpenChange, item, branches }: Props) {
  ...
  const setRow = (branchId: string, patch: Partial<BranchRow>) =>
    setPerBranch((prev) => ({ ...prev, [branchId]: { ...(prev[branchId] ?? { stock: "", low: "" }), ...patch } }));
    ...
    try {
      if (editing && item) {
        ...
        for (const b of branches) {
          ...
          const low = row.low.trim() === "" ? null : parseFloat(row.low);
          if (stock == null && low == null) continue;
          await addToBranchStock(b.id, {
            org_ingredient_id: created.id,
            current_stock: stock,
            reorder_threshold: low,
          });
        }
      }
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              <Label>{t("inventory.catalog.costPerUnit", "Cost / unit (EGP)")}</Label>
          ...
          {!editing && branches.length > 0 ? (
            <div className="space-y-2 rounded-lg border p-3">
              <p className="text-sm font-medium">{t("inventory.catalog.perBranchStock", "Per-branch stock & low-stock level")}</p>
              {branches.map((b) => (
                <div key={b.id} className="grid grid-cols-[1fr_auto_auto] items-center gap-2">
                  <span className="truncate text-sm">{b.name}</span>
                  <Input
                    type="number" min="0" step="0.0001"
                    placeholder={t("inventory.catalog.onHand", "On hand")}
                  ...
                  <Input
                    type="number" min="0" step="0.0001"
                    placeholder={t("inventory.catalog.lowStockLevel", "Low-stock level")}
                    value={perBranch[b.id]?.low ?? ""}
                    onChange={(e) => setRow(b.id, { low: e.target.value })}
                    className="h-8 w-28 tabular"
                  />
                </div>
              ))}
