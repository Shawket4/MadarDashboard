...
import { CHART_AXIS_TICK, CHART_COLORS } from "@/components/app/chart-card";
  ...
  return (
    <ChartTooltipContent
      ...
      payload={payload.map((p) => ({
        name: measures.find((m) => m.key === p.dataKey)?.label,
        dataKey: p.dataKey,
        value: p.value,
        color: p.color,
      }))}
      formatter={(v, name) => {
        const col = measures.find((m) => m.label === name);
        if (!col) return fmtNumber(v);
        // Undo the pounds conversion so the tooltip shows a properly formatted amount.
        return formatCell(col.kind === "money" ? v * 100 : v, col.kind);
      }}
    />
  );
}
  ...
  if (block.row_count === 0) {
    return (
      <EmptyState
        ...
        title={t("basira.noActivity", "No activity in this period")}
      />
    );
  }
        ...
        return (
          <ChartFrame>
            <AreaChart data={chartData} margin={{ top: 8, right: 8, left: 4, bottom: 0 }}>
              ...
              <XAxis dataKey={axisKey} tick={CHART_AXIS_TICK} tickLine={false} axisLine={false} minTickGap={24} />
              <YAxis tick={CHART_AXIS_TICK} tickLine={false} axisLine={false} width={52} />
            ...
            >
              ...
              {block.viz === "row" ? (
                <>
                  ...
                  <YAxis type="category" dataKey={axisKey} width={120} tick={CHART_AXIS_TICK} tickLine={false} axisLine={false} />
                </>
              ) : (
                <>
                  <XAxis dataKey={axisKey} tick={CHART_AXIS_TICK} tickLine={false} axisLine={false} interval={0} angle={chartData.length > 6 ? -25 : 0} textAnchor={chartData.length > 6 ? "end" : "middle"} height={chartData.length > 6 ? 56 : 30} />
                  <YAxis tick={CHART_AXIS_TICK} tickLine={false} axisLine={false} width={52} />
                </>
              )}
              <Tooltip content={<ChartTip measures={measures} />} cursor={{ fill: "var(--muted)", opacity: 0.4 }} />
              {measures.slice(0, 2).map((m, i) => (
                <Bar key={m.key} dataKey={m.key} name={m.label} fill={SERIES[i % SERIES.length]} radius={block.viz === "row" ? [0, 4, 4, 0] : [4, 4, 0, 0]} maxBarSize={48} />
              ))}
            </BarChart>
          </ChartFrame>
        );
...
function ChartFrame({ children }: { children: React.ReactElement }) {
  return (
    <div dir="ltr" className="h-60 w-full">
    ...
    <div className="flex flex-wrap gap-6">
      {measures.map((m) => (
        <div key={m.key}>
          ...
          <p className="mt-1 font-mono text-2xl leading-none font-semibold tracking-tight tabular-nums text-foreground">
...
function TableView({ block }: { block: ResultBlock }) {
  const columns = useMemo<ColumnDef<Record<string, unknown>>[]>(
    () =>
      block.columns.map((c) => ({
        id: c.key,
        header: c.label,
        meta: { label: c.label, numeric: isNumeric(c.kind) },
  ...
  return (
    <DataTable
      columns={columns}
      data={block.rows as Record<string, unknown>[]}
      hideViewOptions
      pageSize={25}
    />
  );
      ...
      <table className="w-full border-separate border-spacing-0.5 text-[10px]">
        <thead>
          <tr>
            <th />
            {colKeys.map((c) => (
              <th key={c} className="px-0.5 pb-1 font-mono font-normal text-muted-foreground tabular-nums">
                {c}
              </th>
            ))}
          </tr>
        ...
        <tbody>
          {rowKeys.map((r) => (
            <tr key={r}>
              <td className="pe-2 text-end font-mono text-muted-foreground tabular-nums whitespace-nowrap">{r}</td>
              {colKeys.map((c) => {
