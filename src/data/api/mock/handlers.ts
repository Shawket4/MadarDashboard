...
const PUBLIC_BRANCHES = [
  {
    ...
    umbrella_open_now: true,
    pickup_open_now: true,
    otp_required: false,
    in_mall_require_location: false,
  },
  {
    // Everything enabled but closed right now — exercises the menu preview +
    // cart-first checkout "ordering is closed" state.
    id: "br_newcairo",
    name: "New Cairo",
    code: "NC",
    in_mall_enabled: true,
    ...
    outside_open_now: false,
    umbrella_open_now: false,
...
export const handlers = [
  ...
  http.get("*/tills/:tillId/spot-checks", ({ params }) => HttpResponse.json(MOCK_TILL_SPOT_CHECKS(params.tillId as string))),
